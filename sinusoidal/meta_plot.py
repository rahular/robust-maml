...
df = pd.concat([df_uniform, df_skewed, df_mm_uniform, df_mm_skewed, df_np_uniform, df_np_skewed], ignore_index=True, sort=False)
df = df[df.k_shot != 40]
df.rename(columns = {'k_shot':'K-Shot'}, inplace = True)
# df = df.loc[df['K-Shot'] == 20]
df.dropna()
plot = sns.relplot(data=df, x="grad_steps", y="loss", hue="Detector", style="Task Distribution", col="K-Shot", kind="line")
sns.despine()
