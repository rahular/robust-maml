...
def conjugate_gradient(
    ...
    if x is None:
        ...
        h1 = (
            Hvp_vec(grad_vec=grad_x, params=y_params, vec=x, retain_graph=True)
            .detach_()
            .mul(lr_y)
        )
        h2 = (
            Hvp_vec(grad_vec=grad_y, params=x_params, vec=h1, retain_graph=True)
            .detach_()
            .mul(lr_x)
        )
        Avx = x + h2
    ...
    for i in range(nsteps):
        # To compute Avp
        h_1 = (
            Hvp_vec(grad_vec=grad_x, params=y_params, vec=p, retain_graph=True)
            .detach_()
            .mul(lr_y)
        )
        h_2 = (
            Hvp_vec(grad_vec=grad_y, params=x_params, vec=h_1, retain_graph=True)
            .detach_()
            .mul(lr_x)
        )
        Avp_ = p + h_2
...
def Hvp_vec(grad_vec, params, vec, retain_graph=False):
    ...
    grad_grad = autograd.grad(
        grad_vec, params, grad_outputs=vec, retain_graph=retain_graph, allow_unused=True
    )
    grad_list = []
...
def general_conjugate_gradient(
    ...
    if x is None:
        ...
        h1 = Hvp_vec(
            grad_vec=grad_x, params=y_params, vec=lr_x * x, retain_graph=True
        ).mul_(lr_y)
        h2 = Hvp_vec(grad_vec=grad_y, params=x_params, vec=h1, retain_graph=True).mul_(
            lr_x
        )
        Avx = x + h2
        r = b.clone().detach() - Avx
    ...
    for i in range(nsteps):
        ...
        h_1 = Hvp_vec(
            grad_vec=grad_x, params=y_params, vec=lr_x * p, retain_graph=True
        ).mul_(lr_y)
        # h_1.mul_(lr_y)
        # lr_y * D_yx * b
