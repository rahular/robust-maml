...
def evaluate(data_gen, meta_model, bert_model, task_bs, inner_loop_steps, inner_lr, num_episodes):
    ...
    for episode in range(num_episodes):  # episode loop = task loop
        ...
        query_error = compute_loss(query_task, bert_model, learner, batch_size=task_bs)
        task_query_error += query_error

        tqdm_bar.set_description("Query Loss : {:.3f}".format(query_error.item()))

    # average the query loss over all tasks
    task_query_error /= num_episodes
    return task_query_error
...
def init_args():
    ...
    parser.add_argument("--dataset_type", help="Type of data to evaluate on: train, test, dev")
...
if __name__ == "__main__":
    ...
    if model_config["is_fomaml"]:
        meta_model = l2l.algorithms.MAML(
            postagger, lr=inner_lr, first_order=True
        )
    else:
